...
function handleRequest(request,response){
	...
	io.sockets.on('connection', function (socket) {
			socket.on('myClick', function (id) {
					var q1 = 'insert into register (itemID, label, price, amount, time_stamp) select inventory.ID, inventory.item, inventory.price, inventory.quantity, now() from inventory where inventory.ID = ';
					q1 += id;
					q1 += '+1;';
			...
			socket.on('getPrices', function () {
					var query1;
					var q5 = 'select sum(price) as price from register';
					var connection = mysql.createConnection(credentials);
					query1=connection.query(q5);
					query1.on('result',function(result){
							socket.emit('updateTotal', result.price);
					});
			...
			socket.on('cleanUp', function(roomNum, payment) {
					var query;
